...
ttgamma_json=$CMSSW_BASE/src/TopLJets2015/TopAnalysis/test/analysis/pps/ttgamma_samples.json
...
inputfileTag=MC13TeV_2017_TTGJets
...
#            -i ${inputfileTESTSELSIGNAL} --tag ${inputfileTagSIGNAL} \
        python $CMSSW_BASE/src/TopLJets2015/TopAnalysis/scripts/runLocalAnalysis.py \
            ...
            -o testsel_2.root --genWeights genweights_${githash}.root \
            --njobs 1 -q local \
            --era era2017 -m ttgamma::RunTTGamma --ch 0 --runSysts;
        ;;
...
#	for file in /eos/user/b/bribeiro/955fa95_ul/*.root
#	do
...
#	echo "regression applied -----------------------------------------------"
   ...
   KIN )
	...
	python scripts/plotter.py ${commonOpts} -j ${samples_json} --signalJson ${signal_json}  -O plots/sel/ul/ --saveLog	--only BDT_output --saveTeX;
