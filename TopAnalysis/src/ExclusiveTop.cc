                     ...
                     Bool_t debug)
{
  ...
  */
  ...
  ht.addHist("newmRP",     new TH1F("newmRP",    ";new M_{RP}; Events",50,0,3000) );
  ht.addHist("newyRP",     new TH1F("newyRP",    ";new y_{RP}; Events",50,-3,3) );
...
*/    
	...
	for(auto &a:photons) {
      ...
      nRPtk=0;
      double goodmRP=10000, goodyRP=10, newgoodmRP=10000, newgoodyRP=10;	  
      int ncomptrks=0;
      ...
      if(!ev.isData && !isSignal) 
	{
	  ...
	  if(ev.isData)
	    ...
	    std::vector<double> newfarCsi0, newfarCsi1;
	    ...
	    if(beamXangle==130) {xi_ratio_45 = xi_ratio_45_130; xi_ratio_56 = xi_ratio_56_130;}
	    if(beamXangle==140) {xi_ratio_45 = xi_ratio_45_140; xi_ratio_56 = xi_ratio_56_140;}
	    if(beamXangle==150) {xi_ratio_45 = xi_ratio_45_150; xi_ratio_56 = xi_ratio_56_150;}

	    goodmRP = 10000;
	    goodyRP = 10;

	    for(int i=0;i<ntks[23];i++) {
	      newfarCsi0.push_back(farCsi0[i]*xi_ratio_45->GetBinContent(xi_ratio_45->FindBin(farCsi0[i])));
	      ht.fill("newxi0",newfarCsi0[i],plotwgts,cats);
	    }

	    for(int j=0;j<ntks[123];j++) {
	      newfarCsi1.push_back(farCsi1[j]*xi_ratio_56->GetBinContent(xi_ratio_56->FindBin(farCsi1[j])));
	      ht.fill("newxi1",newfarCsi1[j],plotwgts,cats);
	    }
		...
		for(int j=0;j<ntks[123];j++)
		  {
		    ...
		    double newmRP = 13000*sqrt(newfarCsi0[i]*newfarCsi1[j]);
		    
		    double yRP = 0.5*TMath::Log(farCsi0[i]/farCsi1[j]);////		    
		    double newyRP = 0.5*TMath::Log(newfarCsi0[i]/newfarCsi1[j]);////		    
		    if(mRP<goodmRP) {goodmRP=mRP; goodyRP=yRP;}
		    if(newmRP<newgoodmRP) {newgoodmRP=newmRP; newgoodyRP=newyRP;}
		    ncomptrks++;		    
		  }
	      }
      ...
      if(passRPcut)
	{
      ...
      ht.fill("newmRP",newgoodmRP,plotwgts,cats);
      ht.fill("newyRP",newgoodyRP,plotwgts,cats);
